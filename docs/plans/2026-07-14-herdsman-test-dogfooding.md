...
- A normal Pi prompt must not wait for a daemon RPC or history read. A missing cache proceeds without Herdsman context.
- Pi presence uses the exact Pi session path. The extension does not send tool-result or final-message telemetry.
- Do not inspect raw session files unless a Herdsman result is demonstrably wrong and diagnosis is required.
...
- Use a disposable Herdsman home for daemon restart, disconnect expiry, owner transfer with pending events, and ownerless replay.
...
| Observer | shell | Runs `herdsman agent list/get/read` without changing pane focus |
...
```bash
herdsman agent get <agent-pane-id> --json
herdsman agent read <agent-pane-id> --limit 20 --json
```
...
Expected: Herdsman preserves the meaningful messages while omitting terminal chrome and unrelated stream text.
...
- [x] **Step 7: Validate Herdsman Agent Skill behavior**
...
- Pi loads the Herdsman skill;
- it checks daemon state before the first explicit Herdsman query;
...
- only Pi A displays an unread Herdsman event indicator;
...
**Objective:** Exercise ownership and unread behavior without modifying normal `~/.herdsman` state.
...
Before claiming, run `/herdsman status` in Pi A and Pi B and trigger one Agent transition.
...
- Pi A's non-owner `off` is a no-op and reports `Herdsman is off`.
...
With Pi A as owner and Pi B off, run five `herdsman agent list --json` calls through `/usr/bin/time -p`. Record each real value and the rows' `updatedAt` values. `agent list` must return cached data; use `agent get/read` only after selecting an exact target that needs current detail.
...
Start Pi before its first cache delivery or restart the disposable daemon and submit a prompt immediately. Confirm the turn starts without a hitch and receives no Herdsman context until a snapshot arrives. Reconnect within grace and verify that the same owner restores cached context through presence registration.
...
Expected: the same Herdr terminal regains or retains `◆ Herdsman` without another `on`, and status continues to identify that terminal under the replacement Pi session. Confirm the renewed presence registers the replacement Pi session's exact path.
...
Mark unavailable runtimes as skipped with the missing executable/version; do not count them as Herdsman failures.
...
- A real Pi RPC process claimed `/herdsman on`. Five normal prompts reached `agent_start` in 4.0, 0.3, 0.3, 0.3, and 0.3 milliseconds. The owner answered `claude` and `codex` from workspace context without tools. No extension error occurred.
- A real temporary Pi B answered `None` while off. After `/herdsman on`, its footer changed to `◆ Herdsman` and the same no-tool prompt answered `Claude` and `Codex`. Pi A's completion arrived as a separate visible Herdsman wake after Pi B's normal answer rather than entering the normal answer.
- Restarting the disposable daemon within grace restored a temporary owner's `◆ Herdsman` state without another claim. The exact replacement Pi session path appeared in the persisted hint.
...
- A prompt sent before owner registration completed proceeded without an extension error or a wait on Herdsman context. Automated call-count coverage separately proves that `agent_start` and `context` perform no daemon request or file read.
...
- Herdsman `0.2.0` using the normal `~/.herdsman` home;
...
$ herdsman agent get wJ:p2
agent target not found: wJ:p2
```

An explicit `herdsman agent list --workspace wJ --session default --json` also returned no agents. `herdsman agent list --all --json` showed the daemon still indexed old workspace `wB`; its stale `wB:p2` Claude row contained the new Claude session history because history discovery selected the latest session with the same working directory. The SQLite agent/workspace rows had not been refreshed since daemon startup.
...
- [x] Herdsman skill selects the proper scope and exact pane target.
