...
**Goal:** Rebuild Herdsman around Herdr agents and agent history so `herdsman agent list/get/read` returns compact, accurate context for running Herdr workspaces, while the daemon keeps push notifications, unread cursors, and cache/index data.

**Architecture:** Herdsman remains a TypeScript daemon + CLI, but Herdr and agent history files are the source of truth. The daemon periodically discovers all running Herdr sessions with `herdr session list --json`, indexes every workspace/agent in those running sessions, subscribes to Herdr agent status events, and stores agent events plus compact history cache in SQLite. CLI and Pi extension talk to the daemon; no user-installed adapter/config is required beyond the Herdsman packages.
...
- Remove old external commands: `herdsman context`, `herdsman snapshot`, `herdsman events`, `herdsman notifications`, `herdsman ack`, `herdsman message-worker`, `herdsman wait-worker`.
- Add external commands: `herdsman agent list`, `herdsman agent get <target>`, `herdsman agent read <target>`.
- CLI defaults to the current Herdr workspace when `HERDR_ENV=1`; `agent list` supports `--all`, `--workspace <id>`, and `--session <name>`.
- CLI requires the Herdsman daemon. If the daemon socket is unavailable, return an error that tells the user to run `herdsman daemon start`. Do not auto-start.
- Herdsman daemon watches only Herdr sessions whose `herdr session list --json` entry has `running: true`. Stopped sessions are out of scope for this rewrite.
...
- `herdsman agent list` returns agent metadata plus one-line compact history: `agent`, `paneId`, `status`, `updatedAt`, `lastUserMessage`, `lastAssistantMessage`.
- `herdsman agent get <target>` returns one agent's metadata plus compact history and `lastToolResult`.
- `herdsman agent read <target> --limit N` returns recent structured messages including compact `tool_result` items.
...
- User accepted deleting existing Herdsman config/DB and rebuilding from zero. Do not preserve compatibility with old SQLite tables or old external RPC names.
...
- Herdr target syntax accepts terminal ids, unique agent names, detected/reported agent labels, and legacy pane ids. Herdsman should resolve targets within the selected workspace unless `--all` is used.
...
4. [RPC and CLI: `herdsman agent list/get/read`](2026-07-08-herdr-agent-history-redesign/04-rpc-cli.md)
...
- In Herdr workspace `wB`, `herdsman agent list --json` returns the Pi pane `wB:p1` and Claude pane `wB:p2` with compact history.
- In Herdr workspace `wB`, `herdsman agent get claude --json` resolves within the current workspace.
- In Herdr workspace `wB`, `herdsman agent read claude --limit 10 --json` returns recent user/assistant/tool_result messages without raw full tool output.
